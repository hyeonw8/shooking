...
- `ShippingFee`에 배송비와 상품 금액을 함께 전달하여 무료배송 문구 표시 제어
...
- 배송비 금액을 표시하며, 상품 금액이 10만 원 이상인 경우 무료배송 문구를 함께 표시
...
> 일반 배송비 표시와 배송비 안내 버튼 클릭 시 툴팁 노출 상태 확인
...
- UI 렌더링 검증
  - 일반 배송비가 정상적으로 표시되는지 확인
  - 무료배송 조건에서 무료배송 문구가 표시되는지 확인
  - 장바구니가 비어 있는 경우 0원이 표시되는지 확인
  - 배송비 안내 버튼 렌더링 확인
...
- `Default` : 일반 배송비 상태 (3000원)
- `Free` : 무료배송 문구가 표시되는 상태
...
- React 19와 Recoil 간 호환성 이슈가 있어 React 18로 조정했습니다.
